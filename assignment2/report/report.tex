...
components given to us, except the ALU. The testbenches are written to test
that expected values are outputted on different inputs. If somthing is wrong, 
assertions generates warnings when running the test. 
When all components were ready and
...
testbench and implemented it using procedures to fill the memory and check 
the register values. The cpu testbench loads the fibonacci program, runs and 
checks for correct register values.
