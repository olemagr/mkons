...
a valid bit for the IF/ID register. This valid bit is written by the control unit as the 
instruction is being fetched from the instruction memory. Checking the current opcode and the 
status register is enough to know if the branch is being taken or not. 
...
\begin{figure}
  ...
  \caption{Pipelined cpu}
  \label{fig:pipelinedcpu}
\end{figure}
